...
static void geo_process_camera(struct GraphNodeCamera *node) {
    ...
    if (node->fnNode.node.children != 0) {
        ...
        if (sky3d_is_active()) {
            sky3d_geo_inject(node->pos, node->focus, node->roll);
        } else {
            static int s_sky_warn_count = 0;
            if (s_sky_warn_count < 3) {
                sky_log("[sky3d] geo_process_camera: sky NOT active (call %d)\n", ++s_sky_warn_count);
            }
        }
        geo_process_node_and_siblings(node->fnNode.node.children);
